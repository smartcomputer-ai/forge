...
export const METHOD_INFO = {
  ...
  "session/managed/start": {
    ...
    description: "Creates a session with an immutable lifecycle controller and/or workflow tools using explicit bound pull/push dispatch, start targets, and Accepted, Joined, or keyed-Promise completion. Profile metadata defaults and the creation-time environment override follow session/start semantics. Retrying an existing session id requires the same managed-creation declaration; an ordinary session cannot be upgraded to managed.",
  },
  "session/read": {
    scope: "universe",
