import { copyFile, mkdir, readFile, writeFile } from "node:fs/promises";
import path from "node:path";
import { fileURLToPath } from "node:url";
import { compileFromFile } from "json-schema-to-typescript";

const packageDir = path.dirname(fileURLToPath(import.meta.url));
const clientDir = path.resolve(packageDir, "..");
const contractDir = path.resolve(
  clientDir,
  "..",
  "..",
  "crates",
  "api",
  "contract",
);
const schemaPath = path.join(contractDir, "api.schema.json");
const methodsPath = path.join(contractDir, "methods.json");
const workflowContractDir = path.resolve(
  clientDir,
  "..",
  "..",
  "crates",
  "temporal-workflow",
  "contract",
);
const workflowSchemaPath = path.join(
  workflowContractDir,
  "workflow.schema.json",
);
const workflowManifestPath = path.join(workflowContractDir, "workflow.json");
const generatedDir = path.join(clientDir, "src", "generated");

const bannerComment = [
  "/*",
  " * Generated by clients/typescript/scripts/generate.mjs.",
  " * Do not edit by hand.",
  " */",
  "",
].join("\n");

const types = await compileFromFile(schemaPath, {
  bannerComment,
  unreachableDefinitions: true,
  additionalProperties: false,
  enableConstEnums: false,
  style: {
    printWidth: 100,
    semi: true,
    singleQuote: false,
    trailingComma: "all",
  },
});

await writeFile(path.join(generatedDir, "types.ts"), types);

const workflowTypes = await compileFromFile(workflowSchemaPath, {
  bannerComment,
  unreachableDefinitions: true,
  additionalProperties: false,
  enableConstEnums: false,
  style: {
    printWidth: 100,
    semi: true,
    singleQuote: false,
    trailingComma: "all",
  },
});

await writeFile(path.join(generatedDir, "workflow-types.ts"), workflowTypes);

const workflowManifest = JSON.parse(
  await readFile(workflowManifestPath, "utf8"),
);
const workflowManifestLines = [
  bannerComment.trimEnd(),
  "export const WORKFLOW_CONTRACT_MANIFEST =",
  `${JSON.stringify(workflowManifest, null, 2)} as const;`,
  "",
];
await writeFile(
  path.join(generatedDir, "workflow-manifest.ts"),
  workflowManifestLines.join("\n"),
);

const manifest = JSON.parse(await readFile(methodsPath, "utf8"));
const methods = manifest.methods ?? [];
const notifications = manifest.notifications ?? [];

for (const entry of methods) {
  if (typeof entry.summary !== "string" || entry.summary.trim().length === 0) {
    throw new Error(`method ${entry.method} is missing summary documentation`);
  }
  if (
    typeof entry.description !== "string" ||
    entry.description.trim().length === 0
  ) {
    throw new Error(
      `method ${entry.method} is missing description documentation`,
    );
  }
}

const seenAliases = new Map();

function typeNameFromRef(entry, method, side) {
  const ref = entry?.schema?.$ref;
  const prefix = "#/definitions/";
  if (typeof ref !== "string" || !ref.startsWith(prefix)) {
    throw new Error(`method ${method} ${side} schema is not a definitions ref`);
  }
  return ref.slice(prefix.length);
}

function methodAlias(method) {
  const parts = method.split(/[\/_-]+/).filter(Boolean);
  if (parts.length === 0) {
    throw new Error("empty method name");
  }
  const alias =
    parts[0] +
    parts
      .slice(1)
      .map((part) => part.slice(0, 1).toUpperCase() + part.slice(1))
      .join("");
  const prior = seenAliases.get(alias);
  if (prior && prior !== method) {
    throw new Error(
      `method alias collision: ${alias} for ${prior} and ${method}`,
    );
  }
  seenAliases.set(alias, method);
  return alias;
}

function q(value) {
  return JSON.stringify(value);
}

function jsDoc(entry, indent = "") {
  const summary = entry.summary.replaceAll("*/", "* /");
  const description = entry.description.replaceAll("*/", "* /");
  return [
    `${indent}/**`,
    `${indent} * ${summary}`,
    `${indent} *`,
    `${indent} * ${description}`,
    `${indent} */`,
  ];
}

const methodLines = [
  bannerComment.trimEnd(),
  'import type * as Api from "./types.js";',
  "",
  "export const METHODS = [",
  ...methods.map((entry) => `  ${q(entry.method)},`),
  "] as const;",
  "",
  "export const METHOD_INFO = {",
  ...methods.flatMap((entry) => [
    `  ${q(entry.method)}: {`,
    `    scope: ${q(entry.scope)},`,
    `    summary: ${q(entry.summary)},`,
    `    description: ${q(entry.description)},`,
    "  },",
  ]),
  "} as const;",
  "",
  "export const NOTIFICATIONS = [",
  ...notifications.map((method) => `  ${q(method)},`),
  "] as const;",
  "",
  "export type Method = (typeof METHODS)[number];",
  "export type NotificationMethod = (typeof NOTIFICATIONS)[number];",
  "",
  "export interface MethodMap {",
  ...methods.flatMap((entry) => {
    const params = typeNameFromRef(entry.params, entry.method, "params");
    const result = typeNameFromRef(entry.result, entry.method, "result");
    return [
      ...jsDoc(entry, "  "),
      `  ${q(entry.method)}: {`,
      `    params: Api.${params};`,
      `    result: Api.${result};`,
      "  };",
    ];
  }),
  "}",
  "",
  'export type MethodParams<M extends Method> = MethodMap[M]["params"];',
  'export type MethodResult<M extends Method> = MethodMap[M]["result"];',
  "",
  "export interface RpcCaller {",
  "  call<M extends Method>(method: M, params: MethodParams<M>): Promise<MethodResult<M>>;",
  "}",
  "",
  "export const rpc = {",
  ...methods.map((entry) => {
    const alias = methodAlias(entry.method);
    const params = typeNameFromRef(entry.params, entry.method, "params");
    const result = typeNameFromRef(entry.result, entry.method, "result");
    return `${jsDoc(entry, "  ").join("\n")}\n  ${alias}(client: RpcCaller, params: Api.${params}): Promise<Api.${result}> {\n    return client.call(${q(entry.method)}, params);\n  },`;
  }),
  "} as const;",
  "",
];

await writeFile(path.join(generatedDir, "methods.ts"), methodLines.join("\n"));

// Ship the canonical contract schema inside the package (exported as
// "@lightspeed/agent-client/schema/api.schema.json") so downstream tooling
// can derive references/validators from the exact schema version it pins.
const schemaOutDir = path.join(clientDir, "schema");
await mkdir(schemaOutDir, { recursive: true });
await copyFile(schemaPath, path.join(schemaOutDir, "api.schema.json"));
await copyFile(
  workflowSchemaPath,
  path.join(schemaOutDir, "workflow.schema.json"),
);
await copyFile(workflowManifestPath, path.join(schemaOutDir, "workflow.json"));
