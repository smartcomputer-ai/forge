# P52: Environment Protocol

> **Update (2026-08-14):** The original `host-protocol` and `host-client`
> vocabulary was renamed to `environment-protocol` and `environment-client`.
> Provider-owned lifecycle objects use `ProviderTarget*` names, while the
> execution plane uses `Environment*` names. Later milestones replaced the
> proposed bridge with `environment-daemon` / `lightspeed-envd`.

**Status**
- Draft

**Progress**
- Added the `environment-protocol` workspace crate for pure protocol data types.
- Implemented G1 data-plane types: shared ids, protocol version, `EnvironmentScope`,
  `EnvironmentCapabilities`, `ByteChunk`, `EnvironmentPath`, common error payloads,
...
- Implemented G2 controller-plane types: controller handshake, lifecycle method
  ...
  the shared `EnvironmentTransport`/`EnvironmentDataConnection` handoff.
- Added controller-plane serde fixtures and round-trip tests.
- Added the `environment-client` workspace crate with a small JSON-RPC core, WebSocket
...
Define the external protocol used to manage provider targets and operate an
environment execution endpoint.
...
The crate names describe the environment extension boundary:

- `environment-protocol`: pure protocol types, method names, serde fixtures, and
  compatibility rules for both the environment data plane and the provider
  controller plane
- `environment-client`: one reusable client crate for both data-plane and controller
  calls, even if a consumer only uses one side
- `environment-daemon`: passive execution endpoint implementing the data plane
- `sandbox-controller`: later production service implementation of the
  controller protocol

P52 focuses on `environment-protocol`, one `environment-client`, the minimal controller
...
Concretely, `environment-protocol` should borrow these Codex protocol parts:
...
Implement the environment data plane:
...
- return a `EnvironmentDataConnection` that the data-plane client can connect with
...
## `environment-protocol` Crate Shape

`environment-protocol` should be a pure types crate. It should not open sockets, own
...
- protocol version
- ids such as `ProviderTargetId`, `EnvironmentConnectionId`, and `ProcessId`
- `EnvironmentScope`
...
Prefer plane-scoped imports such as `environment_protocol::data::fs::ReadFileParams`
or `environment_protocol::control::targets::CreateTargetParams`. Keep top-level
...
- create/attach responses that return `EnvironmentDataConnection`

Keep JSON-RPC transport envelopes out of `environment-protocol` unless a typed
...
configuration in `environment-client` connect options. Do not put credentials in
...
The controller plane should live in `environment-protocol` beside the data plane, but
...
```rust
pub struct EnvironmentDataConnection {
...
connection spec to create a `environment-client` data-plane connection, builds a
...
`environment-client` should be one reusable client crate:

- depends on `environment-protocol`
...
- `RemoteEnvironmentFileSystem` implements `FileSystem` over `environment-client`
- `RemoteProcessExecutor` implements `ProcessExecutor` over `environment-client`
...
```text
host:sandbox_123
  -> endpoint: wss://host.example/v1/environment-protocol
...
Do not implement the environment data plane as MCP.
...
classification. That belongs in `environment-protocol`. MCP tools can call a
controller or SDK layer later.

## Implementation Order

### [x] G1. Add Data-Plane `environment-protocol` Types

- Add a workspace crate for pure protocol types.
- Define shared ids, protocol version, `EnvironmentScope`, capability types, common
...
### [x] G2. Add Controller-Plane `environment-protocol` Types
...
- Define `EnvironmentTransport` and `EnvironmentDataConnection` as the handoff from
...
- Support minimal controller methods and returning `EnvironmentDataConnection`.
...
- Use `environment-client` controller calls to obtain a `EnvironmentDataConnection`.
...
- Document how a Codex-style exec server maps to `environment-protocol`.
- Document how Fabric/smolvm maps to `environment-protocol` without an in-guest Lightspeed
  binary.
