...
- Renumbered from P100 when its generic workflow-tool substrate was split
...
- Synchronized 2026-07-28 after P100 completion: uses the settled
  `WorkflowTool*` vocabulary, `WorkflowTool::Emitted`, Accepted completion,
  and `read_tool_emissions`; no legacy transport vocabulary remains.
- Builds on **P100 (Workflow Emissions And Workflow Tools)**, **P92 (Unified
...
The agent reports a disposition through one Work-owned workflow tool:

```text
work_report(outcome = complete | blocked, ...)
```

P100 admits that workflow tool with the managed session and gives it the fixed
`AgentWorkWorkflow` controller. A valid call becomes a typed, log-backed
workflow-tool emission in the managed session's log.
...
reading that run's durable workflow-tool emissions through P100's pull
operation —
...
AgentWorkWorkflow
  ...
  -> pull and reconcile the run's log-backed workflow-tool reports
...
| P100 workflow tool | schema-validated semantic fact emitted by a session for one fixed admitted receiver workflow |
...
`WorkflowTool::Emitted` fact in the session log. Nothing is pushed to Work
...
- the run's `lightspeed.work.report.v1` workflow-tool emissions, read through
  P100's `read_tool_emissions(receiver_endpoint, session_id, run_id)`
  operation;
...
and no dedupe set for report-delivery signals; reconciliation is idempotent
because the activity result is recorded once per cycle and repeated
reconciliation of an already-reconciled cycle is a no-op. P101 adds only the
Work payload and handler; declaration, schema validation, log events, and the
pull read belong to P100. If Work ever needs mid-run reports (it should not),
that is P100's deferred push slice, not a Work-specific transport.
...
4. Named-profile execution with P100's `work_report` workflow tool declared by
   the managed-session setup path.
...
- generic workflow-tool infrastructure beyond P100;
...
- function tools with a fixed admitted receiver per binding;
- typed `WorkflowTool::Emitted` session events with deterministic invocation
  identity;
- the boundary-complete `read_tool_emissions` pull operation.

P101 consumes that substrate and must not fork or specialize it.

## Work's Workflow Tool

During managed-session setup, `AgentWorkWorkflow` declares one P100 workflow
tool:
...
The tool is present because of Work's controller-bound workflow-tool
declaration, not a public Work feature flag and not an independently
installable Work tool bundle. A standalone session has no Work controller and
therefore cannot grant itself a meaningful `work_report` binding.

This binding does not make Work the receiver for every workflow tool in the
session. The same managed session may independently bind messaging or later
service tools to their own workflow receivers.
...
A successful workflow-tool result means the report was durably recorded, not
that Work has already accepted or acted on the disposition.
...
only that bounded terminal body; workflow-tool reports never arrive by signal
and the workflow never performs CAS or session-log I/O.
...
initialize
  ...
  -> create/reopen managed session with profile + P100 work_report workflow tool
...
run_terminal
  -> reconcile the exact completed run, pulling its workflow-tool emissions
...
durable workflow-tool emissions by pull. A duplicate terminal emission for an
...
The activity reads the run's workflow-tool emissions through P100's
`read_tool_emissions(receiver_endpoint, session_id, run_id)` operation and
validates that each report comes from the
...
### Duplicate or stale workflow-tool emission
...
anchors. P100 emits one deterministic workflow-tool invocation; repeated
reconciliation reads return the same emission and derive the same
disposition.

### Session continues as new

The notify-intent lives on the stored run record and is rebuilt. Workflow-tool
...
P100 owns all engine/tools changes for workflow tools. Do not add Work
semantics to `CoreAgentState`; it records generic workflow-tool invocations and
run state, while `AgentWorkWorkflow` interprets `WorkReportV1`.
...
### Slice 2: Bind Work to the P100 workflow-tool substrate
...
- [ ] Decode and validate the typed payload through the Work-owned
      reconciliation activity over P100's `read_tool_emissions`, not in the
...
- [ ] Create/reopen one profile-based managed session with
      `close_on_terminal = false` and the P100 `work_report` workflow tool.
...
4. A valid `work_report(complete)` P100 workflow-tool invocation observed at
   the matching terminal boundary completes Work.
...
8. Work reuses the existing run-request, notify-intent, session admission,
   cancellation, and P100 workflow-tool substrate; it adds no Work message
