...
| `bot_events` | `(universe_id, bot_id, event_id)` + unique `(universe_id, bot_id, seq)` | the envelope row exactly as today: `seq`, trigger, kind, source, occurred at, `ref`, `prompt_ref`, routed session, sender bot, hops, `reply_to`, `in_reply_to`, media, `tools`, `notify`, write-once outcome columns |
