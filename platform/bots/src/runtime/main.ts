...
for (const role of roles) {
  if (role !== "workflows" && role !== "activities") {
    throw new TypeError(`unknown bots role: ${role}`);
  }
}
...
console.log(`bots: starting ${roles.join(", ")} in one process`);
...
try {
  if (running.length === 1) {
    ...
    if (!stopping) {
      throw new Error(`bots ${stopped} role stopped while other roles were running`);
    }
