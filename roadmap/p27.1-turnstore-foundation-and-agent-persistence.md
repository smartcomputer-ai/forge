...
- G5 completed: Attractor-facing storage contracts/types added in `forge-attractor` with `forge-turnstore`-only dependencies.
- G6 completed: deterministic integration tests/docs added for persistence modes and queryability across memory/fs backends.
...
### [x] G5. Add Attractor-facing storage contracts (interfaces only)
- Work:
...
### [x] G6. Add deterministic integration tests and docs
- Work:
...
## Deferred follow-ups (tracked in CXDB adapter/runtime phases)
- Agent checkpoint snapshot persistence as explicit turn records (`spec/04-cxdb-integration-spec.md` Section 4.3) is deferred beyond this pre-runtime foundation scope.
- Production msgpack numeric-tag encoding for persisted payloads (`spec/04-cxdb-integration-spec.md` Section 3.6, `spec/cxdb/type-registry.md`) is deferred to CXDB-backed writers; current local backends persist JSON envelopes deterministically for parity testing.
