...
use crate::error::{EnvironmentClientError, EnvironmentClientResult};
        ...
        R: DeserializeOwned,
    {
        ...
        loop {
            let message = self
                ...
                .recv()
                .await?
                .ok_or(EnvironmentClientError::TransportClosed)?;
            ...
            let Some(response_id) = message.get("id").and_then(Value::as_u64) else {
                return Err(EnvironmentClientError::InvalidMessage(
                    "response missing numeric id".to_owned(),
                ));
            };
            if response_id != id {
                return Err(EnvironmentClientError::InvalidMessage(format!(
                    "unexpected response id {response_id}, expected {id}"
                )));
            }

            if let Some(error) = message.get("error") {
                return Err(EnvironmentClientError::Protocol(parse_error(error)?));
            }

            let result = message.get("result").ok_or_else(|| {
                EnvironmentClientError::InvalidMessage("response missing result".to_owned())
            })?;
            return serde_json::from_value(result.clone()).map_err(Into::into);
        }
    }

    pub async fn notify<P>(&mut self, method: &str, params: &P) -> EnvironmentClientResult<()>
    where
        P: Serialize + Sync,
    {
        self.transport
...
fn parse_notification(message: &Value) -> EnvironmentClientResult<Option<JsonRpcNotification>> {
...
fn parse_error(value: &Value) -> EnvironmentClientResult<EnvironmentProtocolError> {
    if let Ok(error) = serde_json::from_value::<EnvironmentProtocolError>(value.clone()) {
        return Ok(error);
    }
    ...
    Ok(EnvironmentProtocolError::new(
