//! Transport implementations for environment protocol clients.
...
use crate::{
    error::{EnvironmentClientError, EnvironmentClientResult},
...
impl JsonRpcTransport for WebSocketTransport {
    ...
    async fn recv(&mut self) -> EnvironmentClientResult<Option<Value>> {
        loop {
            ...
            let message =
                message.map_err(|error| EnvironmentClientError::Transport(error.to_string()))?;
            match message {
                Message::Text(text) => {
