//! Reusable conformance checks for environment data-plane implementations.
...
#[derive(Clone, Debug)]
pub struct EnvironmentDataConformanceOptions {
    pub initialize: InitializeParams,
    pub expected_capabilities: EnvironmentCapabilities,
...
#[derive(Debug, Error)]
pub enum EnvironmentDataConformanceError {
    #[error(transparent)]
    Client(#[from] EnvironmentClientError),
    ...
    #[error("environment data conformance check failed: {0}")]
    Check(String),
}
...
pub async fn assert_environment_data_conformance<T>(
    mut client: EnvironmentDataClient<T>,
    options: EnvironmentDataConformanceOptions,
) -> Result<(), EnvironmentDataConformanceError>
where
    T: JsonRpcTransport + Send + 'static,
{
    ...
    let connection = RemoteEnvironmentConnection::new(client, initialized.capabilities)
    ...
    if fs_context.fs.access_policy() != FileAccessPolicy::FullReadWrite {
        return Err(check(
            "writable environment filesystem was not exposed as read-write",
        ));
    }
    ...
    let contents = "environment data conformance\n";
...
fn check(message: impl Into<String>) -> EnvironmentDataConformanceError {
    EnvironmentDataConformanceError::Check(message.into())
}
