...
pub struct ReadFileParams {
    ...
    /// Byte offset to start reading from. Environment endpoints advertising
    /// `filesystem_ranged_read` honor it; others ignore it and return the
    /// full file.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub offset: Option<u64>,
    /// Maximum bytes to return. Environment endpoints advertising `filesystem_ranged_read`
...
pub struct ReadFileResponse {
    pub data: ByteChunk,
    /// Total size of the file in the environment, independent of the returned range.
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
pub struct CopyParams {
    pub source_path: EnvironmentPath,
    pub destination_path: EnvironmentPath,
    #[serde(default)]
    pub recursive: bool,
}
...
/// Bounded recursive text search executed natively by the environment endpoint.
///
/// The environment endpoint performs traversal and matching locally and returns only bounded
/// matches plus scan statistics, so a broad search does not become per-file
/// transfer over the data plane. Every limit is mandatory: an endpoint must stop
...
/// Bounded recursive file enumeration executed natively by the environment endpoint.
///
...
pub struct GlobFilesResponse {
    pub matches: Vec<EnvironmentPath>,
