...
use forge_agent::{AnthropicProviderProfile, ExecutionEnvironment, SessionConfig, SubmitOptions};
use forge_llm::Role;
use serde_json::json;
use std::sync::Arc;
use support::live::{
...
async fn anthropic_live_create_then_edit_file_smoke_applies_expected_side_effects() {
    ...
    .await
    .expect("anthropic live create/edit smoke should pass");
}
...
async fn anthropic_live_read_file_truncation_smoke_preserves_full_tool_call_end_output() {
    ...
    .await
    .expect("anthropic live truncation smoke should pass");
}
...
async fn anthropic_live_shell_timeout_smoke_returns_timeout_tool_result() {
    ...
    .await
    .expect("anthropic live shell-timeout smoke should pass");
}
...
async fn anthropic_live_submit_with_options_smoke_applies_request_overrides() {
    ...
    run_with_retries(|_| {
        ...
        async move {
            ...
            let override_marker = "ANTHROPIC_LIVE_OVERRIDE_MARKER";
            let provider_options = json!({ "anthropic": {} });
            ...
            assert_eq!(last.provider.as_deref(), Some("anthropic"));
    ...
    .await
    .expect("anthropic live submit_with_options smoke should pass");
}
