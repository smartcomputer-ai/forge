...
impl EmissionId {
    ...
    /// Tool-invocation delivery uses the durable invocation id directly so receiver
    /// dedup and session-log identity share one stable key.
    pub fn for_tool_invocation(invocation_id: &WorkflowToolInvocationId) -> Self {
        Self(invocation_id.as_str().to_owned())
    }

    /// Best-effort per-key cancellation fact for one keyed completion
    /// promise of a promise-bearing workflow-tool invocation.
    pub fn for_invocation_cancellation(
        invocation_id: &WorkflowToolInvocationId,
        completion_key: &str,
    ) -> Self {
        Self::from_parts(
            b"invocation_cancellation",
            &[invocation_id.as_str().as_bytes(), completion_key.as_bytes()],
        )
    }
}
...
pub enum EmissionIdError {
    #[error(
        "emission id must use emission:sha256:<64 lowercase hex> or wti:sha256:<64 lowercase hex> format: {value}"
    )]
...
impl EmissionEnvelope {
    ...
    pub fn invocation_cancellation(
        universe_id: Uuid,
        session_id: SessionId,
        log_seq: EventSeq,
        invocation_id: WorkflowToolInvocationId,
        ...
        let emission_id = EmissionId::for_invocation_cancellation(&invocation_id, &completion_key);
        Self {
            emission_id,
            producer: EmissionProducer::Session {
                universe_id,
...
fn is_canonical_emission_id(value: &str) -> bool {
    let hex = value
        ...
        .or_else(|| value.strip_prefix("wti:sha256:"));
    hex.is_some_and(|hex| {
...
mod tests {
    ...
    #[test]
    fn tool_invocation_reuses_the_durable_invocation_id() {
        let invocation_id = WorkflowToolInvocationId::new(format!("wti:sha256:{}", "a".repeat(64)));
        let emission_id = EmissionId::for_tool_invocation(&invocation_id);

        assert_eq!(emission_id.as_str(), invocation_id.as_str());
        assert_eq!(
            EmissionId::parse(emission_id.to_string()).expect("parse tool emission id"),
            emission_id
        );
    }
