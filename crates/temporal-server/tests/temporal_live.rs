...
async fn run_fleet_send_report_back_live_client(
    ...
    let spawn_result = executor
        ...
        .await
        .map_err(|error| anyhow::anyhow!("{error}"))?;
    if spawn_result.status != ToolCallStatus::Succeeded {
        let detail_ref = spawn_result
            .error_ref
            .as_ref()
            .or(spawn_result.output_ref.as_ref());
        let detail = match detail_ref {
            Some(detail_ref) => blobs.read_text(detail_ref).await?,
            None => "<no result detail>".to_owned(),
        };
        anyhow::bail!(
            "mode-i setup spawn failed with {:?}: {detail}",
            spawn_result.status
        );
    }
